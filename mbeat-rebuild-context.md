...
- Every post type needs to be **status-showable**: not just visible/hidden (moderation status), but tracking *what's currently true about the real-world situation* — e.g. a death announcement moves through `awaiting_body` → `body_available` → `upcoming_janazah` → `janazah_in_progress` → `completed` (becomes historical record; see "deathAnnouncement form + schema" and "janazah lifecycle status control" progress log entries below for why the original 3-stage vocabulary was split); a blood request moves through `open` → `fulfilled`/`expired`.
...
## Progress log — Google auth + mandatory profile completion (in progress, WIP)
- Priority shifted here before the `deathAnnouncement` form (noted below as the previous "immediate next step") because posting requires a signed-in, profile-complete user first. Work on feature branch `feature/auth-google-login` (off `develop`).
- New migration: `supabase/migrations/20260813024543_create_profiles_table.sql` — creates `profiles` (1:1 with `auth.users`), first pass holding only the mandatory onboarding fields: `full_name`, `phone`, `locality`. Directory fields (`profession`/`status`/`search_vector` etc., per the earlier Find Now design) are deliberately deferred to when that module is actually built. RLS: own-row only (`profiles_select_own`/`_insert_own`/`_update_own`) — no public directory read yet. **Pushed and applied to the live Supabase project** (verified via `supabase migration list` — local/remote timestamps match).
- New `src/features/auth/`: `AuthContext.jsx` + `AuthContextObject.js` + `useAuth.js` (session via `supabase.auth.getSession()`/`onAuthStateChange`, exposes `signInWithGoogle`/`signOut`), `ProtectedRoute.jsx` (redirects unauthenticated visitors to `/login`), `ProfileCompletion.jsx` (loads the caller's `profiles` row, shows an onboarding form and inserts one if missing, blocks children until it exists — this is the mandatory one-time onboarding step from the Auth strategy section above).
- New `src/pages/LoginPage.jsx` — Google sign-in button only so far. Phone+OTP (the other confirmed auth path) not started yet.
- `src/App.jsx` rewired with `react-router-dom` (`BrowserRouter`): `/login` route, `/` gated behind `ProtectedRoute` → `ProfileCompletion`, currently rendering an inline placeholder feed screen (not yet the planned `pages/FeedPage.jsx`). `react-router-dom` added to `package.json`.
...
## Progress log — create-post route + bilingual i18n + UX guideline alignment (done)
- `/post/new` (`NewDeathAnnouncementPage`) gave `DeathAnnouncementForm` its permanent home — closes out the previous "immediate next step." Route wired into `App.jsx` alongside `/`, both gated behind `ProtectedRoute` → `ProfileCompletion`.
- `react-i18next` wired up across auth, feed, and death-announcement screens: `src/lib/i18n/` (`en.json`/`ta.json`, `setLanguage()`), `components/ui/LanguageToggle.jsx`, `preferred_language` synced to the `profiles` table (new migration `20260817120000_add_profiles_preferred_language.sql`) for signed-in users and `localStorage` for guests.
- Closed gaps against the (then-new) UI/UX guidelines section of `CLAUDE.md`: skeleton loading states (`components/ui/Skeleton.jsx`) instead of blank screens, labeled form inputs, a retry-able feed error state, an actionable empty-feed state (CTA to create the first post), `components/ui/BackButton.jsx` reused across non-root pages, and a `--color-brand` Tailwind `@theme` token replacing hardcoded `green-600` usages.
- This work landed straight on `develop` (commit `b1bf5f3`), not a feature branch — scaffolding-adjacent per the `develop`-is-the-catch-all rule.

## Progress log — death announcement detail page + Islamic Guide pattern (done)
- Work done on feature branch `feature/death-announcement-enhancements` (off `develop`).
- `PostDetailPage.jsx` (route `/post/:id`) + `features/deathAnnouncement/DeathAnnouncementDetail.jsx` — full read view of a death announcement (deceased info, janazah/body/burial locations, formatted date/times), reusing `feedApi.fetchPostById()` (now joins `death_announcement(*)`).
- First instance of the locked **Islamic Guide** content pattern: `features/islamicContent/` (`ContentGuideDetail.jsx`, `guides/janazahPrayer.js`, `guides/index.js`) + route `pages/IslamicGuidePage.jsx` (`/guide/:slug`) — static, dawah-oriented reference content, linked from the death announcement detail page ("How to pray Janazah").
- New field: `death_announcement.death_datetime` (migration `20260817181148_...`) — when the person died, distinct from `janazah_datetime`. Locked in the naming table.

## Progress log — deceased's photo: upload, thumbnail, full-size viewer (done)
- Same feature branch. New Storage bucket `post-photos` (migration `20260818083228_create_post_photos_bucket.sql`), paths `<uploader_user_id>/<file>` per the locked naming-table convention.
- `DeathAnnouncementForm` gained a photo picker (client-side type/size validation against `DEATH_ANNOUNCEMENT_PHOTO_ACCEPTED_TYPES`/`_MAX_BYTES` in `deathAnnouncementSchema.js`) that uploads to `post-photos` and stores the public URL on `death_announcement.photo_url`.
- `PostCard.jsx` shows the photo as a feed thumbnail (`getThumbnailUrl()` — a per-type lookup, deliberately not yet the full `PostCard.<type>.jsx` dispatch mechanism noted as deferred in the feed-engine-v1 entry above); `DeathAnnouncementDetail` shows it full-width and opens a full-size viewer via new `components/ui/ImageLightbox.jsx` on tap.

## Progress log — editing, body_location, timezone fix, lifecycle-aware feed order (done)
- Same feature branch. `DeathAnnouncementForm` now serves both create and edit (`editingPost` prop): `pages/EditDeathAnnouncementPage.jsx` (route `/post/:id/edit`, author-only — checks `post.author_id === user.id`), with an edit link surfaced on `DeathAnnouncementDetail` for the owner. Editing updates `posts.title`/`description` and all `death_announcement` extension fields, but never re-infers `lifecycle_status` (that's now a separate explicit control — see next entry) and preserves the existing photo unless a new one is picked.
- New field: `death_announcement.body_location` (migration `20260818092515_...`) — where the body/family can be visited, distinct from `janazah_location`. Locked in the naming table (label: "Body location"); `janazah_location`'s display label was also reworded to "Prayer location" for clarity (schema/DB column name unchanged).
- Timezone fix: `datetime-local` inputs are timezone-less wall-clock strings, but `timestamptz` columns round-trip as UTC — `toDatetimeLocalValue()`/`fromDatetimeLocalValue()` helpers in `DeathAnnouncementForm.jsx` convert correctly through the local offset both directions (previously a naive `.toISOString()` would have silently shifted times on edit).
- `feedApi.fetchPosts()` now orders by `lifecycle_updated_at` (falling back to `created_at`) instead of plain `created_at` — a status change (e.g. "Janazah started") bumps a post back toward the top of the feed, not just brand-new posts. A content-only edit (typo fix) doesn't touch `lifecycle_updated_at`, so it deliberately can't be used to bump a post.

## Progress log — janazah lifecycle status control + smart form defaults (done)
- Same feature branch (`feature/death-announcement-enhancements`, currently up to date with `origin`, 4 commits ahead of `develop`, all in-scope for the branch name — no drift).
- **Lifecycle vocabulary expanded** (locked, see naming table + "Lifecycle status" section above): the original `upcoming_janazah → janazah_in_progress → completed` split into `awaiting_body → body_available → upcoming_janazah → janazah_in_progress → completed`, so the feed/detail page can distinguish "don't come yet, body still in transit" from "visitors welcome now." Defined in `features/deathAnnouncement/lifecycleStatus.js`.
- `DeathAnnouncementDetail` gained a `LifecycleStatusControl` — author-only pill buttons to advance status, wired through `PostDetailPage.handleLifecycleStatusChange` as an optimistic update (reflects immediately, rolls back on write failure) against `posts.lifecycle_status` (the existing RLS trigger auto-logs to `lifecycle_status_history` and stamps `lifecycle_updated_at`, per the RLS-policies work — no new backend logic needed).
- Smart defaults in `DeathAnnouncementForm`: `death_datetime` pre-fills to "now" on a new post (most announcements are posted shortly after death); a new `body_not_yet_available` checkbox (UI-only, never persisted) drives `inferInitialLifecycleStatus()` explicitly — a blank `body_location` is deliberately *not* treated as "body unavailable" (often just means "at home, didn't feel the need to name it").
- `CLAUDE.md` also gained a "branch-scope discipline" working-style rule in this window (commit `666c1b8`), prompted by scope drift once seen on `feature/auth-google-login` — unrelated to this branch's content but committed alongside it.

## Immediate next step
This branch (`feature/death-announcement-enhancements`) looks ready to PR into `develop` — it's a coherent, complete chunk (detail page, photo, editing, lifecycle control) with no unrelated scope. After that merges: `main` is still just the original scaffold commit (no release cut yet — nothing has been promoted past `develop`), so decide whether/when a first promotion to `main` makes sense. Beyond that, the next module/feature (Find Now, blood requests, reputation system) is open — none of those are started. Phone+OTP login also remains deferred until an SMS provider is set up in the Supabase dashboard.

## Notes for whoever picks this up next
- Working in this session: branch `feature/death-announcement-enhancements` (off `develop`), repo cloned at whichever machine's local path (see multi-system note above) — always confirm current branch before assuming `main`. `main` is well behind `develop`/feature branches (only the original scaffold commit) — this is believed intentional (pre-launch, nothing promoted yet), not an oversight, but wasn't independently confirmed with Syed.

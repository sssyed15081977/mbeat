...
- Every post type needs to be **status-showable**: not just visible/hidden (moderation status), but tracking *what's currently true about the real-world situation* — e.g. a death announcement moves through `awaiting_body` → `body_available` → `upcoming_janazah` → `janazah_in_progress` → `completed` (becomes historical record); a blood request moves through `open` → `fulfilled`/`expired`.
...
## Progress log — bilingual i18n + UX guidelines pass (done)
- Work done on feature branch `feature/auth-google-login` (off `develop`).
- Wired up `react-i18next` end to end: `src/lib/i18n/index.js` (setup + `setLanguage()`), `en.json`/`ta.json` resource files, `components/ui/LanguageToggle.jsx`. Language preference persists to `localStorage` for guests and syncs to the new `profiles.preferred_language` column (migration `20260817120000_add_profiles_preferred_language.sql`) once signed in. Applied across auth, feed, and death-announcement screens — no hardcoded UI strings left in the touched files.
- Closed gaps against the new UI/UX guidelines section added to `CLAUDE.md` in this same commit: skeleton loading states (`components/ui/Skeleton.jsx`) instead of blank screens, labeled form inputs, a retry-able feed error state, an actionable empty-feed state, a reusable `components/ui/BackButton.jsx` for non-root pages, and a `--color-brand` design token replacing hardcoded `green-600` usages.
- This is also where the "Every page is mobile-first and bilingual" standing requirement and the `BackButton`-on-every-non-root-page rule were locked into `CLAUDE.md`.

## Progress log — death announcement detail page, Islamic Guide pattern, death_datetime (done)
- Work done on feature branch `feature/auth-google-login` (off `develop`).
- Feed cards now link to a new `pages/PostDetailPage.jsx`, which joins `death_announcement` data via a new `fetchPostById()` in `feedApi.js` and renders through `features/deathAnnouncement/DeathAnnouncementDetail.jsx`.
- New migration `20260818081148_add_death_announcement_death_datetime.sql` adds `death_announcement.death_datetime` (optional, distinct from `janazah_datetime`) — surfaced in both the form and detail view.
- New reusable **Islamic Guide** pattern: `features/islamicContent/` (`ContentGuideDetail.jsx`, `guides/`), routed at `pages/IslamicGuidePage.jsx` (`/guide/:slug`). Seeded with one bilingual (EN/TA) guide, `guides/janazahPrayer.js` ("How to Pray Janazah" — Salat al-Janazah procedure), explicitly flagged in-code as pending scholarly review before being treated as authoritative. Linked from the death-announcement detail page — first concrete instance of the dawah-content surfacing goal.
- Both new terms (Islamic Guide, `death_datetime`) locked in the naming glossary above.

## Progress log — deceased's photo: upload, feed thumbnail, full-size viewer (done)
- Work done on feature branch `feature/auth-google-login` (off `develop`).
- New migration `20260818083228_create_post_photos_bucket.sql` creates the shared Supabase Storage bucket `post-photos` (RLS-scoped so a user can only write to their own `<uploader_user_id>/<file>` path) — deliberately shared across post types, not death-announcement-specific, since any future post type with a photo can reuse it.
- `DeathAnnouncementForm` uploads to it; the photo shows as a feed-card thumbnail via a small per-type lookup added to the generic `PostCard.jsx`, and as a tap-to-enlarge image on the detail page via a new generic `components/ui/ImageLightbox.jsx`.
- Bucket naming/path convention locked in the naming glossary above.

## Progress log — editing, body_location, timezone fix, lifecycle-aware feed order (done)
- Work done on feature branch `feature/auth-google-login` (off `develop`).
- **Editing**: authors can now edit their own death announcement — new `/post/:id/edit` route → `pages/EditDeathAnnouncementPage.jsx`; `DeathAnnouncementForm` now doubles as both the create and edit form; an edit link is shown to the post's owner on the detail page.
- **`body_location`**: new migration `20260818092515_add_death_announcement_body_location.sql` adds `death_announcement.body_location` (where the body/family can be visited for condolences — distinct from `janazah_location`, the prayer venue). The "Janazah location" UI label was also reworded to "Prayer location" for clarity — schema/column name unchanged. Both locked in the naming glossary above.
- **Timezone fix**: death/janazah datetimes were being sent to Postgres as naive strings and misinterpreted as UTC instead of local time (e.g. 9:00 AM saved and redisplayed as 2:30 PM IST) — fixed in the form/detail conversion logic.
- **Feed ordering**: `useFeed()`/`feedApi.js` now order by `lifecycle_updated_at` over `created_at`, so a real status change (janazah in progress, completed) resurfaces a post, while a cosmetic content edit doesn't bump it — the lifecycle-aware ranking called for in the "Lifecycle status" section above.
- Photo/thumbnail now render full-width on both the feed card and detail page instead of a small square crop.

## Progress log — janazah lifecycle status control + smart form defaults (done)
- Work done on feature branch `feature/death-announcement-enhancements` (off `develop`) — first commit on this branch name; prior work above was all done on `feature/auth-google-login`.
- **Lifecycle vocabulary split**: `upcoming_janazah` was split into `awaiting_body` → `body_available`, so the feed can distinguish "body not yet visitable" from "visitors welcome now." Full stage list is now `awaiting_body` → `body_available` → `upcoming_janazah` → `janazah_in_progress` → `completed`. No migration needed — `posts.lifecycle_status` is plain `text` with no DB-level CHECK enum, so this is an app-level vocabulary change only (`features/deathAnnouncement/lifecycleStatus.js`). Updated in the naming glossary above.
- **Owner-only status control**: the detail page (`PostDetailPage.jsx` / `DeathAnnouncementDetail.jsx`) now lets the post's author advance `lifecycle_status` directly, with an optimistic UI update, reusing the existing `posts_update_own` RLS policy (no new policy needed).
- **Smart form defaults**: `death_datetime` now defaults to "now" on the create form; the post title auto-fills from the deceased's name until the announcer edits it directly. "Not yet available" (i.e. `awaiting_body`) is only ever set from an explicit checkbox — deliberately never inferred from an empty `body_location` field, since a blank field usually just means "at home, no need to state it," not "not available"; inferring the latter would misinform visitors.

## Immediate next step
Core death-announcement flow (create, edit, detail view, photo, lifecycle status, bilingual UI) is now feature-complete end to end. Not yet started: **Find Now** (search/directory tab — `profiles`/`entities`/`entity_members`, its own nav destination, `ProfileCard`/`EntityCard`) and phone+OTP login (deferred until an SMS provider is configured in the Supabase dashboard). Also still open: `PostCard.<type>.jsx` dispatch for the feed's summary card (currently a generic shell only — type-specific rendering lives on the detail page instead), and no test runner is configured yet. Branch `feature/death-announcement-enhancements` is clean and pushed to `origin`; not yet confirmed whether it has an open PR against `develop`.

## Notes for whoever picks this up next
- Working in this session: branch `feature/death-announcement-enhancements` (off `develop`), repo cloned at whichever machine's local path (see multi-system note above) — always confirm current branch before assuming `main`.

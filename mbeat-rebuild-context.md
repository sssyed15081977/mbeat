...
- **RLS behavior not yet tested against the live project.** A full test-case list was worked out in chat (SQL Editor role/JWT-claim simulation via `set local role` + `set local request.jwt.claims`, plus optional REST API spot-checks with real test-user JWTs) covering: anon sees only published; author sees/edits own regardless of status; cross-author writes blocked; `moderation_status`/`author_id` edits silently no-op for non-service-role; lifecycle changes auto-log to history; direct history inserts blocked. Deferred — Syed wants to run this later himself.

## Immediate next step
Test the RLS policies against the live Supabase project (see test-case list above — needs two real test users created first, since `posts.author_id` etc. have FK constraints into `auth.users`), then start the `features/deathAnnouncement/` form + schema that writes into this schema.
